...
def extract_phone_cnn(
    ...
    store:              open echoframe.Store to write into. CNN features
                        are always written to a per-model store (see
                        extract_phone_cnn_for_models), so unlike
    ...
    batch_size:         segments per forward-pass batch.
                        compute_cnn_features_batch loads every segment's
                        audio into one batch when this is left None and
                        gpu=False - an explicit default avoids loading all
                        of phones' audio into memory at once
...
def extract_phone_cnn_for_models(
    ...
    are opened below `store_root`, then the cached model is unloaded and
    the store is closed after each extraction. When `gpu` is true,
    unreferenced CUDA allocations are also released before the next model
    is loaded.

    Returns a dictionary mapping each model name to its store path.
    '''
    return _extract_phone_cnn_for_models(
        phones,
        model_names,
        collar=collar,
...
def extract_flemish_phone_cnn_for_models(
    ...
    inventory exposed by ``flemish_phones.phraser_phones`` is extracted
    through the same single-model workflow used for the Netherlandic phone
    inventory. Each cached model is unloaded and its store is closed after
    extraction.
